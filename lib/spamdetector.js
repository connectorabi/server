...
module.exports = (ipAddress, userOptions) => {
  var options = {
    suspendInterval: 60    // defaul 1 hour
  }
  ...
  if (ipList[ipAddress] == undefined) {
    ...
    if (ipList[ipAddress].attemptCount > 300 && diffSeconds < 60) {
    ...
    } else if (ipList[ipAddress].attemptCount > 900 && diffSeconds > 60 && diffSeconds < options.suspendInterval) {
      ipList[ipAddress].suspended = dateAddSecond(new Date(), options.suspendInterval)
      ipList[ipAddress].attemptCount = 0
      ipList[ipAddress].firstAttempt = null

      return options.suspendInterval
    } else if (ipList[ipAddress].attemptCount > 300 && diffSeconds > 60) {
      ipList[ipAddress].attemptCount = 0
      ipList[ipAddress].firstAttempt = null
      return false
    } else {
